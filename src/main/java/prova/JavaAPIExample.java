...
//		}
		...
		//assegno dei tag per ognuna
		for(Track tracciaCorrente : insiemeTracce){
			String nomeTraccia = tracciaCorrente.getName();
			String artistaTraccia = tracciaCorrente.getArtist();
			String mbidSTRING = tracciaCorrente.getMbid();
			
			Collection<String> tags = Track.getTags(artistaTraccia, nomeTraccia, null);
			for(String tagCorrente : tags){
				
				String tagName = tagCorrente.getName();
//				if((tagName.equals(""))||(tagName.isEmpty())) tagName="NULL";
				System.out.println(tagName);
//				queryString = "merge(t:Traccia{Traccia:{Traccia}}) "+ 
//						"merge(tag:Tag{Tag:{Tag}})"+
//						"merge(tag)-[:GENERE]-(t)";
//				parameters.put("Traccia", nomeTraccia);
//				parameters.put("Tag", artistaTraccia);
